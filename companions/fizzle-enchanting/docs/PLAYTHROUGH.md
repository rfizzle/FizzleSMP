# Fizzle Enchanting — Manual QA Playthrough

End-to-end checklist for verifying a Fizzle Enchanting build before tagging a release. Walks one tester from a fresh world to every MVP system: shelf tiers, stat readout, Eterna 50, library upgrade, all three tomes, Warden tendril, `infused_breath`, foreign-enchant rolls, and the integration tooltips. Treat each top-level section as a gate — if a check fails, file the regression before moving on.
...
- [ ] Server jar `fizzle-enchanting-<version>.jar` placed in `mods/`; client jar matches **byte-for-byte** (the mod is `environment: "*"` — version skew desyncs on first table interaction).
- [ ] `config/fizzle_enchanting.json` is generated on first launch with `configVersion: 1`. Delete it and reboot to confirm regeneration.
- [ ] Server log shows `Fizzle Enchanting initialized` and **no** `clamped …` warnings on the default file.
- [ ] `/fizzleenchanting reload` (op) replies with the `command.fizzle_enchanting.reload.ok` translation.
- [ ] `/fizzleenchanting stats <self>`, `/fizzleenchanting library <self> dump`, `/fizzleenchanting give-tome <self> scrap` parse and execute (stubs reply, but the surface must be live).
...
- [ ] Place a Fizzle enchanting table; open it. The `FizzleEnchantmentScreen` shows the one-line stat readout `E: 0  Q: 0  A: 0  R: 0  C: 0` below the three slots (toggleable via `config.enchantingTable.showLevelIndicator`).
...
- [ ] Lower `config.enchantingTable.maxEterna` to `30`, `/fizzleenchanting reload`, reopen the table → readout caps at `E: 30`.
...
### 3.1 Basic library
- [ ] Craft `library` (vanilla-shape recipe, ships as `data/fizzle_enchanting/recipe/library.json`). Place it.
...
- [ ] Set `config.library.ioRateLimitTicks: 20`, `/fizzleenchanting reload` → second hopper insert within 20 ticks is dropped (T-4.5.3).

### 3.2 Ender upgrade
- [ ] Place a Fizzle enchanting table next to the Basic Library. Slot the Library item into the table; surround with shelves to satisfy the `ender_library` recipe's stat requirements (see `data/fizzle_enchanting/recipe/enchanting/ender_library.json`).
...
- [ ] Craft a `prismatic_web` (Zenith-shape recipe, ships under `data/fizzle_enchanting/recipe/prismatic_web.json`).
...
### 5.1 Scrap Tome
- [ ] Craft `scrap_tome` (Zenith-shape recipe, ships at `data/fizzle_enchanting/recipe/scrap_tome.json`).
...
- [ ] Craft via the table: input `scrap_tome` against an `improved_scrap_tome` recipe (ships at `data/fizzle_enchanting/recipe/enchanting/improved_scrap_tome.json`); requires the table's `infused_breath`-tier stat block.
...
### 5.3 Extraction Tome
- [ ] Craft via the table (`data/fizzle_enchanting/recipe/enchanting/extraction_tome.json`).
...
- [ ] Craft `infused_breath` via the table (`data/fizzle_enchanting/recipe/enchanting/infused_breath.json`) using `dragon_breath` as input. Confirm it can **only** be obtained this way (no crafting-table recipe exists).
...
### 10.1 EMI / REI / JEI
- [ ] With **EMI** loaded, open the EMI panel and search "fizzle". Two categories appear: "Fizzle Enchanting — Shelves", "Fizzle Enchanting — Tomes". Each `fizzle_enchanting:enchanting` and `keep_nbt_enchanting` recipe has its inputs, output, stat requirements, and XP cost rendered.
...
### 10.2 Jade
- [ ] With **Jade** loaded, look at a Fizzle enchanting table → probe shows the 5 stats (`E/Q/A/R/C`) computed via `EnchantingStatRegistry.gatherStats`.
...
Open the advancement screen (`L` by default) and verify the **Fizzle Enchanting** tab is populated. As the playthrough progresses, expect to unlock:

- [ ] `root` — granted on inventory-pickup of any Fizzle shelf.
...
- [ ] Edit `config/fizzle_enchanting.json` while the server is running (e.g. flip `enchantingTable.maxEterna` to `25`).
- [ ] `/fizzleenchanting reload` (perm 2) → `command.fizzle_enchanting.reload.ok` reply; **no server restart**.
...
- [ ] After all of the above, server log contains zero `Caused by` stacks attributable to Fizzle Enchanting.
