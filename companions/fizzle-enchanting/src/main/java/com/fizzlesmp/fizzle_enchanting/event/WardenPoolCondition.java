 ...
 * {@link FizzleEnchanting#getConfig()} on every {@link #test(LootContext)} call rather than
 * baking the chance at MODIFY time, so operators running {@code /fizzleenchanting reload} see
...
public record WardenPoolCondition(Kind kind) implements LootItemCondition {
     ...
     * {@code FizzleEnchantingRegistry.register()}. Registration must run before the loot-
    ...
    private static volatile Supplier<FizzleEnchantingConfig> configSupplier = FizzleEnchanting::getConfig;
     ...
     */
    static void overrideConfigSupplier(Supplier<FizzleEnchantingConfig> supplier) {
        configSupplier = supplier;
    }

    /** Restores the default supplier ({@link FizzleEnchanting#getConfig()}). */
    static void resetConfigSupplier() {
        configSupplier = FizzleEnchanting::getConfig;
    }
     ...
     */
    float resolveChance(int lootingLevel, FizzleEnchantingConfig config) {
        return switch (kind) {
