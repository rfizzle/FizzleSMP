...
- Modifiers are namespaced (`fizzle_difficulty:*`) and can be identified, removed, or debugged.
...
|------|-----------------|---------------|--------------------|
| **Time** | Player's cumulative online playtime | Per-player persistent level (0–250) | `fizzle_difficulty:time_*` |
| **Distance** | Mob's distance from world spawn | Block distance at spawn time | `fizzle_difficulty:distance_*` |
| **Height** | Mob's Y-level relative to sea level | Absolute Y offset from baseline | `fizzle_difficulty:height_*` |
...
- **Item type:** Custom registered item (`fizzle_difficulty:shatter_shard`) with a custom texture. Appears in a dedicated creative tab.
...
- **Adds:** Fizzle Difficulty companion mod (unified replacement)
...
| Aspect | HMIOT | Fizzle Difficulty |
|--------|-------|-------------------|
...
| Aspect | RpgDifficulty | Fizzle Difficulty |
|--------|---------------|-------------------|
