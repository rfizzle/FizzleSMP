...
1. **Gather Data**
   - Read all `plugins/*.md` files and extract every mod with `Status: included`.
   - Read `docs/compatibility-matrix.md` for known hard/soft conflicts.

2. **Known Conflict Check**
   - Cross-reference all included mods against the hard and soft conflict tables.
   - Report any included mod pairs that appear in the hard conflict list — these are **blockers**.
   - Report any included mod pairs that appear in the soft conflict list — these are **warnings** with their noted resolutions.

3. **Functional Overlap Detection**
   - Group included mods by category.
...
5. **Missing Matrix Entries**
   - For every pair of included mods that does NOT appear in the compatibility matrix at all, list them as **untested pairs** that should be verified.
...
6. **Report**
   ...
   - If there are zero included mods, say so and remind the user to set mod statuses.