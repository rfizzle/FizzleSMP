...
- **Conflicts:** None known. Ships `config/inventoryprofilesnext/rules-v2.fizzlesmp.txt` overriding `@creative_menu_order` to delegate to `@item_name`. The native rule uses `::search_tab_index`, which freezes clients on first use while building every creative tab (upstream blackd/Inventory-Profiles#440). On IPN 1.20.5+ the file **must** match `rules-v2.*.txt`; plain `rules.txt` is silently ignored.
