...
| Spectrum | Tech Reborn | Spectrum bundles optional Tech Reborn integration recipes (grinder/industrial grinder for Spectrum ores) using an outdated recipe JSON format — bare string ingredients and `results`/`tank` keys instead of Tech Reborn's current `outputs`/`fluid` keys with `SizedIngredient` objects | **Resolved** — Spectrum's Tech Reborn integration is disabled via `IntegrationPacksToSkipLoading: ["techreborn"]` in `modpack/config/Spectrum.json5`. This eliminates all 20 broken recipe errors. To restore working recipes on a server, place `Spectrum-TechReborn-Recipe-Fix.zip` (in `modpack/config/paxi/datapacks/`) into the world's `datapacks/` folder — world-level datapacks override mod-bundled data. Paxi global datapacks cannot override mod recipes. |
