...
| When Dungeons Arise | Structory | Both add overworld structures (WDA: large castles/keeps, Structory: small-medium atmospheric ruins) | Complementary — different scale structures. WDA adds large landmarks, Structory adds small ambient structures. No overlap. |
| When Dungeons Arise: Seven Seas | Repurposed Structures | Both add ocean structures (Seven Seas: large vessels, RS: shipwreck variants) | May generate near each other in oceans. No functional conflict — different structure types. Sparse Structures can manage density. |
| When Dungeons Arise: Seven Seas | Hopo Better Underwater Ruins | Both add ocean content (Seven Seas: surface vessels, Hopo: underwater ruins) | Complementary — different depth levels (surface vs seafloor). No conflict. |
...
| Sparse Structures | All new structure mods (WDA, Seven Seas, Explorify, MVS, Philip's Ruins, Tidal Towns, Better Archeology) | Sparse Structures controls generation frequency for all structure mods | Essential with many structure mods — tune Sparse Structures config to prevent oversaturation. |
