...
| TieredZ | RPG Series weapons (Archers, Rogues & Warriors, Arsenal) | TieredZ applies random tiers to all tools/weapons; RPG Series adds custom weapon types | Should be complementary — verify TieredZ modifiers apply correctly to RPG Series weapons. |
...
| The Aether | All Overworld/Nether/End worldgen mods (Terralith, Tectonic, Incendium, Nullscape) | Aether is a separate sky dimension with its own portal system | No conflict — completely independent dimension. Does not modify Overworld, Nether, or End generation. |
| The Aether | Accessories | Aether embeds Accessories beta.48 in its jar (standalone is beta.53). Aether registers its accessory slots (gloves, capes, pendants, rings, shields) via conditional sub-packs (`packs/accessories` and `packs/accessories_override`) injected through `ServerPacksSourceMixin`, but these sub-packs **fail to load** on Fabric — Aether items cannot be equipped in any accessory slot without a fix | **Resolved** — `Aether-Accessories-Fix.zip` datapack (in `modpack/config/paxi/datapacks/`) maps Aether items to standard Accessories slots: gloves→hand, capes→cape, shields→back, pendants→necklace, rings→ring, misc→charm. Aether tooltip still says "Slot: Gloves" but items equip in the Hand slot. |
| The Aether | Lootr | Aether adds loot chests in dungeons; Lootr instances loot per player | Should be compatible — verify Lootr instances Aether dungeon chests correctly. |
