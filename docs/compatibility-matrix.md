...
| Supplementaries | CTOV, Repurposed Structures | Supplementaries' auto-generated road signs in villages trigger expensive `locate structure` calls across all village types. With 12+ village variants from CTOV and Repurposed Structures, this causes 20–45 second tick lag spikes during village chunk generation. | **Resolved** — disable road sign generation in `config/supplementaries-common.json` (`sign_post_enabled: false` or `road_sign.enabled: false`). The signpost block remains usable for players; only auto-generated village road signs are disabled. |
...
| LI: Better Archeology | Better Archeology | Addon designed specifically for Better Archeology; requires Loot Integrations | Designed integration — enhances loot in Better Archeology structure chests. |
...
| Sparse Structures | All new structure mods (Explorify, MVS, MES, MNS, MSS, Philip's Ruins, Tidal Towns, Better Archeology) | Sparse Structures controls generation frequency for all structure mods | Essential with many structure mods — tune Sparse Structures config to prevent oversaturation. |
...
| The Aether | Accessories | Aether embeds Accessories in its jar; Accessories is also installed separately in the pack | Should be compatible — Fabric's jar-in-jar system uses the higher version. Monitor for duplicate class warnings in logs. |
